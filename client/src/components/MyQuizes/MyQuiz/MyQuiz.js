...
function MyQuiz({ quiz }) {
  ...
  //  const socket = io("http://localhost:3001")
    let newGame = await dispatch(createGame(gameData,history))
   // console.log("day la id: " + newGame._id)
    let leaderboardData = { gameId: newGame._id, playerResultList: [] }
    
    let newLeaderboard = await dispatch(createLeaderboard(leaderboardData))
